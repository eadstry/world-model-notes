...
## 视觉 Tokenization：自回归范式的瓶颈与突破
无论是哪种子范式，视觉 tokenization 都是决定自回归 VWM 性能的核心瓶颈。离散 token 的引入使得世界模型训练与语言模型共享了成熟的 Transformer 架构，但代价巨大：
1. **信息损失**：将连续视觉信号量化为离散 token 必然导致信息损失。VQ-VAE 在压缩比 16x 时图像质量尚且可接受，但用于视频时（尤其是需要精确物理交互的场景），细节损失会严重影响下游任务。
2. **序列长度爆炸**：一张 256x256 图像经 VQGAN 编码后可产生 256 个 token（16x16 latent grid），这意味着 10 秒 30fps 的视频将产生约 76800 个 token。对 O(n^2) 复杂度的 transformer 而言，这是一个恐怖的计算负担。
3. **token 利用率问题**：视频帧间存在极大的冗余——相邻帧的 token 序列可能 90% 以上是相似的。如何设计高效的 token 调度策略（如 stride tokenization, keyframe-based conditioning）是当前研究热点。
当前趋势正在向**连续隐空间的自回归**方向探索——即放弃离散 tokenization，直接在连续嵌入空间中做自回归预测，从而避免量化损失。
## 动作条件注入机制的技术对比
自回归 VWM 中，动作信号 $c_t$ 如何注入是一个核心设计选择。
| 机制 | 实现方式 | 优势 | 劣势 | 代表工作 |
|------|---------|------|------|---------|
| **Additive Embedding** | 动作 token embedding 与视觉 token embedding 相加 | 实现简单，计算开销小 | 模态融合弱 | Genie |
| **Cross-Attention** | 在每个 transformer 中让视觉 token 对动作 token 做 cross-attention | 模态交互充分 | 计算开销大，需额外参数 | iVideoGPT |
| **Adaptive Layer Norm** | 用动作信号调节 transformer 层的归一化参数 | 保持架构简洁的同时实现有效条件控制 | 条件容量有限 | VideoWorld |
| **Prefix-based** | 将动作 token 作为视觉序列的前缀 | 利用 KV-cache 优势 | 无法实现帧间动态调节 | MineWorld |
...
2. **Conditional Reset**：每 K 帧用 ground truth 状态进行一次条件重置（teacher forcing reset）。这本质上是一种训练策略，无法在推理时使用。
