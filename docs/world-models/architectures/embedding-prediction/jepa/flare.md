...
**Controllable Simulation（可控推演）**：FLARE化仿真"是隐式的——策略在生成每一步动作时，内部预测token已经过想象"未来。人类视频共训练使模型从大规模无标注数据中学习环境的物理规律和操作语义，从而在未见任务上表现更好。
## 代码实现要点

- 极简修改：在VLA扩散Transformer中添加少量预测token
- 未来潜在表示对齐损失作为辅助训练目标
- 与无动作标注人类自我中心视频进行共训练- 支持单臂和人形机器人桌面操作两个基准
- NVIDIA GEAR团队

## 关键创新。
- 隐式世界建模：预测token集成在策略网络中，非独立模块
- 极简架构修改：仅添加少量token到VLA模型
- 人类视频共训练：利用无标注数据显著提升泛化能的- 单次演示+共训练即可泛化到未见几何物体
- 26%性能提升超越先前策略学习基线
