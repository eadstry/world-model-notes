...
**Controllable Simulation（可控推演）**：动作控制信号可作为额外条件注入预测模型，实现特征空间中的动作条件未来预测。这种语义级规模拟"可用于机器人任务规划、物体交互预测等。
## 代码实现要点

- DINOv2作为固定视觉编码器提供语义特征空间- 轻量级时序预测器在DINO特征空间中的训练
- 动作条件注入机制支撑可控预测
- 基于DINO预测特征的下游任务适配

## 关键创新。
- 利用DINO特征的预测的预言"能力进行世界建模
- 语义级预测：在对象感知的特征空间中预测未来- 自监督预训练世界知识的免费迁移- 轻量预测器设。
## 代表性结。
- 在视频预测和未来状态推断任务中验证有效相- 机器人操作场景中提供有效的语义级未来预测
