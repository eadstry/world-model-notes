...
传统的世界模型通常只在单一模态或单视角下工作，为BEVWorld 首次在统一BEV空间中对多模态传感器数据进行联合建模。通过在共享空间表示中集成环视图像和点云数据，该模型能够更完整地理解驾驶场景的几何结构和语义信息，为下游感知、预测和规划任务提供更丰富的世界动态先验。
...
- 支持在nuScenes等标准自动驾驶基准上进行训练和评。
- 提供预训练权重和推理脚本，可用于未来场景生成和下游任务评。

## 关键创新。

- 首次提出在统一BEV潜在空间中对多模态传感器数据进行联合建模，实现环视图像和LiDAR点云的双向编码解码
- 通过射线投射渲染实现自监督的多模态重建，无需额外标注
- 潜在BEV序列扩散模型在紧凑的空间表示中进行时序预测，效率高且生成质量。
- 高层动作条件控制使模型适用于自动驾驶的交互式仿。

## 代表性结。

- 在自动驾驶基准测试中展示了逼真的未来场景生成能。
- BEV潜在表示能够有效提升下游感知和运动预测任务的性能
- 多模态重建质量超越了单模态方法的视觉保真。
- 时序预测在场景级别保持了时空一致。
