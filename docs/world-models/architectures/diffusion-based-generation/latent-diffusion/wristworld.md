...
### 核心思想
WristWorld 针对机器人操作中的视角变化问题，提出通过 4D 世界模型生成腕部视角（wrist-view）视频。核心思想是：给定第三视角（external-view）相机图像和机器人动作序列，使用 4D于D 空间 + 时间）世界模型生成对应的腕部相机视角视频帧。这解决了机器人操作中腕部相机视角缺失或不可靠时的感知问题。

### 技术架。
- **Vision Encoding（视觉编码）**：使用双路编码器分别处理第三视角 RGB 图像和机器人本体状态（关节角度、末端执行器位姿）。通过隐式 3D 表示（如 Triplane 何3D Gaussian）将 2D 图像提升的3D-aware 特征。动作序列通过时序编码器处理。
- **Knowledge Learning（知识学习）**：核心是 4D 扩散模型——在 3D 隐式表示 + 时间维度的D）上建模场景动态，通过交叉视角注意力（cross-view attention）学习第三视角和腕部视角之间的几何变换关系，从而在外推腕部视角时保了3D 一致性。
- **Controllable Simulation（可控仿真）**：给定第三视角图像和动作序列，模型生成对应的腕部视角视频，支持机器人在没有腕部相机的情况下想象"腕部视图。
...
- 使用 Triplane 表示动3D Gaussian 作为中间 3D 表示
- 扩散模型的Video Latent Diffusion Model，条件注入包含action tokens + 第三视角 latent
- 训练数据需同时包含第三视角和腕部视角的同步录制视频

### 代表性结。
- 在机器人操作场景生成腕部视角视频，PSNR 的SSIM 优于单视角插值方。
- 腕部视图与机械臂动作的一致性（IoU 评估）验证了 3D 几何一致。
