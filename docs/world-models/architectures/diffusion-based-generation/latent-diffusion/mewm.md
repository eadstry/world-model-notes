...
MEWM 的关键洞察是：机器人面临的任务通常有重复性的模式（如多次拿起同类型物体），利用记忆可以避免从而想象"，直接参考过往经验加速预测。记忆增强使世界模型在少数样本（few-shot）场景中表现突出。
...
**Controllable Simulation（可控模拟）**：记忆使世界模型能快速适应新场景——首次遇到unknown 物体时从记忆中找到最相似物体，以其经验作为先验。

## 代码实现要点

暂无开源代码。基准Diffusion WM + Memory bank + retrieval-augmented denoising。在 Robosuite 于MetaWorld 上评估。

## 关键创新。

1. **记忆增强的世界模型*：检索增强生成（RAG）风格的世界模型
2. **Few-shot 快速适应**：利用记忆加速对新场景的适应
3. **经验复用**：记忆使 agent 不重复学习类似情。
4. **Memory-conditioned 扩散**：记忆作为去噪过程的条件

## 代表性结。

在MetaWorld ifew-shot 操控任务中（一5 个示范），MEWM 的记忆增强使世界模型预测精度比无记忆 baseline 型35%。在反复遇到类似物体时，记忆检索使预测速度提升 2 倍（更少的去噪步骤）。
