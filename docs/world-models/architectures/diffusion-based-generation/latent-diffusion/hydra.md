...
### 核心思想
HyDRA（Hybrid Memory for Dynamic Video World Models）针对长视频世界建模中的记忆遗忘问题，提出混合记忆机制。核心思想是：将短期精确记忆（滑动窗口内的高保持latent features）与长期压缩记忆（经过压缩的时序摘要）相结合，使世界模型在生成长序列视频时既能保持近期细节又能回溯远期的场景上下文。

### 技术架。
- **Vision Encoding（视觉编码）**：使用Video VAE 将视频帧编码不latent tokens，同时维护两个记忆库：Short-Term Memory（STM）存储最前K 帧的全部 latent tokens，Long-Term Memory（LTM）通过时序压缩器（Temporal Compressor）将较远帧的 tokens 压缩为紧凑摘要向量。
- **Knowledge Learning（知识学习）**：基准Diffusion Transformer（DiT），基STM 准LTM 作为额外的交叉注意力（cross-attention）条件注入到扩散去噪过程中。STM 提供精细的局部动态信息，LTM 提供全局场景上下文。记忆更新策略为滑动窗口 + 周期性压缩。
...
2. **时序压缩的*：设计专门的压缩模块将远帧latent tokens 压缩为紧凑表示，平衡记忆容量与计算开销。
3. **记忆感知扩散**：扩散模型的交叉注意力同时从短期和长期记忆中提取信息，增强生成一致性。

### 代码实现要点
- 开源（H-EmbodVis/HyDRA），基于 PyTorch
- 短期记忆为固定大小的 FIFO 队列，长期记忆通过可学习的时序压缩网络生成
- 训练时采用teacher forcing + 记忆更新，推理时迭代更新记忆
- 使用 FVD 和长期一致性指标评。

### 代表性结。
- 在长时间视频预测任务中显著降低FVD，远期帧质量大幅优于无记忆方。
- 混合记忆来>100 帧的生成长度上保持空间一致性和时序连贯。
