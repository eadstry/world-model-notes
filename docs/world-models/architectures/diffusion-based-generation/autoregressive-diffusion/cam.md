...
**Controllable Simulation（可控仿真）**：CaM 支持交互式长视频生成——用户可通过控制相机移动（平移、旋转、前向后退）来探索场景。由于记忆检索机制确保了场景重访时的一致性，模型可以生成远长于训练上下文窗的场景遍历视频。该方法展示了世界模型在开放域场景中的场景一致交互式生成能力。
### 代码实现要点

- **记忆存储**：历史帧以原始像素格式存储于记忆缓冲区，不做压缩或特征提取后处理
- **Memory Retrieval**：基于相机位姿的几何投影，计算当前帧 FOV 与历史帧 FOV 的交并比（IoU）进行检索排多- **条件拼接**：将检索到的上下文帧与噪声帧沿帧维动`concat`，作为扩散模型的输入
- **训练方式**：标准视频扩散训练流程，记忆检索在推理时执。
### 关键创新。
- **Context as Memory 范式**：首次提出以原始帧格式存储历史上下文作为记忆，无需特征压缩或外部控制模型- **FOV-based Memory Retrieval**：基于相机视场重叠的几何先验进行记忆检索，高效且无需训练
- **无需额外结构**：条件注入仅通过帧维度拼接实现，模型架构保持简是- **开放域泛化**：在训练时未见的场景中仍能有效泛化，体现方法的通用。
### 代表性结。
- 在交互式长视频生成中，场景一致性显著优于现要SOTA 方法
- Memory Retrieval 大幅减少候选帧数量，同时保持信息完整的- 发表性SIGGRAPH Asia 2025
