...
::: tip 范式定义
Autoregressive Diffusion 是 Diffusion-based Generation 与 Sequential Generation **两种范式的交汇点**。它保留了自回归的序列结构（逐帧/逐块生成），但每一步不是一个 token 的预测，而是一个完整的**扩散去噪过程**——在自回归的框架内嵌入扩散的质量优势。
:::
...
- **自回归部分**：推理时，模型在自回归循环中运行——每一帧由扩散模型根据上一帧和当前动作生成
...
### Matrix-Game (2025)
Matrix-Game 是该路线中**结构化控制**方向上的代表。它专门针对需要精确控制的世界模拟场景。
...
Hunyuan-GameCraft 代表**中国在该方向上的投入**。其特点：
...
## 自回归扩散的核心技术挑战
### 1. 计算成本的两难
这是自回归扩散的最大难题。假设每帧扩散需要 4 步去噪（已经是非常激进的蒸馏水平），30fps 的游戏需要每秒 120 步扩散推理。每一步扩散需要一次完整的前向传播——对于 DiT 级别的模型来说，这在高分辨率（如 512x512）下几乎不可能实时。
...
纯 AR 模型的 exposure bias 问题（训练使用 ground truth 输入，推理使用模型自身预测作为输入）。在自回归扩散中，这个问题被**放大**了——因为每一步的输出是扩散模型生成的，本质上就带有随机性。
GameNGen 的解决方案是**噪声增强训练**：在训练时向条件帧添加一定量的噪声，使模型学会从「不完美」的条件帧中恢复。这相当于在扩散模型内部做了一层隐式的数据增强，但代价是训练更难收敛。
### 3. 动作信号的时序分解
在自回归扩散框架中，一个动作信号对应一帧的扩散生成。但现实中的动作可能跨越多个视频帧（例如，按下「前进」键持续 300ms，跨越约 10 帧）。如何处理**动作持续时长与帧率的不匹配**？
当前策略包括：
- **Action Chunking**：将连续的动作序列分割为与帧率对齐的片段
