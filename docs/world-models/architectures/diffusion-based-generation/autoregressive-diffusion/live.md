...
**Knowledge Learning（世界知识学习）**：核心是循环一致性训练框架：(1)从真实帧出发，以动作条件进行前向自回归推演；(2)将终端帧作为起点，以逆动作条件进行反向生成；(3)在重建的初始状态上计算扩散损失。这个前向反向循环显式约束了长程误差的有界性。渐进式训练课程逐步增加推演长度，避免直接训练长序列的不稳定。
...
- 渐进式训练课程：逐步增加推演长度以稳定训系- 统一视角涵盖不同训练方法（教师蒸馏、分布匹配等的- 不依赖教师模型推理，计算效率更高

## 关键创新。
- 循环一致性目标：前向→反向的闭环约束显式限制长程误差累积
- 完全消除教师模型依赖：无需蒸馏即可实现长程稳定生成
- 统一理论视角：将已有方法纳入统一框架分析
- 渐进式训练课程：稳定长序列训练过的- 超训练视野泛化：生成远超训练推演长度的稳定视。
## 代表性结。
- 长程基准上达到SOTA性能
- 训练视野外的稳定视频生成能力
- 高质量视觉输出和精确的动作跟踪- 相比教师蒸馏方法计算成本更低
