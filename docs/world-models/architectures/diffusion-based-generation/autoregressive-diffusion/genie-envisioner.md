...
(1) **GE-Base**：大规模指令条件的视频扩散模型，在结构化的潜在空间中捕捉真实世界机器人交互的空间、时间和语义动态。这是一个理解机器人操作世界模基础模型"。
...
**Knowledge Learning（世界知识学习）**：GE-Base 在大规模机器人交互数据上学习操纵世界的空间时间-语义动态。Flow-matching 解码器从潜在表示中学习动作映射，仅需极少监督即可泛化到不同具身形态。
**Controllable Simulation（可控推演）**：GE-Act 从潜在表示解码可执行动作，GE-Sim 作为动作条件神经仿真器提供闭环rollouts。EWMBench 基准确保生成质量、物理一致性和指令-动作对齐的标准化度量。
## 代码实现要点

- GE-Base：大规模指令条件视频扩散模型
- GE-Act：Flow-matching 解码器（潜在表示 了动作轨迹运- GE-Sim：动作条件神经仿真器（闭环策略开发）
- EWMBench：标准化基准（视觉保真度 + 物理一致性+ 指令-动作对齐的- 智元机器人出品，全面开。
## 关键创新。
- 首个统一策略学习、评估、仿真的世界基础平台
- Flow-matching 解码器实现潜在表示到动作的轻量映了- 跨具身形态的可泛化策略推理（少监督）
- 闭环神经仿真支持策略迭代开的- EWMBench 标准化评估基。
## 代表性结。
- 跨多种具身形态的策略泛化
