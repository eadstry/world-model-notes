...
ApolloScape 定义一26 个语义类别的全景标注，远多于 KITTI 分3 类，更接近于 nuScenes 分23 类。数据集的标注任务涵盖：场景解析（像素级语义分割）、车道线分割和D 车辆检测、立体视觉和自定位。ApolloScape 还提供完整的轨迹数据，支持对车辆运动跟踪的研究。尽管如今其在数据噪声和标注一致性方面不同Waymo 在 nuScenes，但 ApolloScape 作为中国自动驾驶数据领域和全栈标注的先驱工作具有重要的历史意义。

## 关键特征
- **数据规模**: 140K+ 标注帧一6 个语义类别、中国城市道路环。
- **数据模式**: RGB 图像、激光雷达点云、GPS/IMU 轨迹、像素级标注
- **主要指标**: mIoU（语义分割）、mAP3D 检测）、车道检测准确率
- **领域**: 自动驾驶感知、中国道路场景、全景标。
