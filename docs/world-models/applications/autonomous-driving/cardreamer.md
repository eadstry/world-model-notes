...
CarDreamer 提供了三个关卡（Level 1-3）的渐进式训练协议：Level 1 在简单直线道路场景中训练，Level 2 扩展到城镇道路交叉口，Level 3 涉及复杂的多车道、多交通参与者交互场景。该平台每40 个不同的 CARLA Town 环境中收集了超过 300,000 条驾驶轨迹，覆盖晴天、雨天、夜晚等多种天气和光照条件。CarDreamer 的一个关键创新是否增量专家混合"策略——利用来自多个专家的数据来引导世界模型的学习。

## 关键特征
- **数据规模**: 300K+ 驾驶轨迹规0 在 CARLA 环境的 个难度等级、多种天气光照
- **数据模式**: CARLA 仿真数据（RGB、深度、语义分割）、车辆状态、专用强化学习轨迹
- **主要指标**: 驾驶分数、路径完成率、违规统计、世界模型预测质。
- **领域**: 世界模型驱动的自动驾驶、渐进式学习、CARLA 评测

## 与世界模型的关系
CarDreamer 是首个明确定义了"面向自动驾驶"的世界模型训练框架，其三级渐进学习协议为世界模型的自动驾驶任务提供了结构化的训练路径。该工作展示了Dreamer 系列世界模型在 CARLA 环境中的直接应用，验证了世界模型从仿真到驾驶的可行性。CarDreamer 还提出了"世界模型质量 = 下游驾驶性能"的评估哲学，推动社区以决策为导向评价世界模型。

## 代表性用例
- UC Davis team 使用 CarDreamer 训练 DreamerV3 的自动驾驶变。
- 作为世界模型在 CARLA 中的标准化训练平。
- 与世界模型驾驶评测竞赛直接相关的平台
- 推动作Dreamer 系列模型向自动驾驶领域的迁移
