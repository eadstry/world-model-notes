...
DrivingDojo 包含三大模块：场景采集引擎（在 CARLA 和真实数据中提取驾驶场景）、世界模型训练器（支路Dreamer、IRIS 等多种世界模型架构）、以及交互式评估器（允许世界模型在模拟环境中"驾驶"并根据表现进行调整）。DrivingDojo 收集在 CARLA 一12 个城镇的多样化驾驶数据以在 nuScenes 的多场景数据，为世界模型提供了丰富的"练习素材"。该框架的创新在于其闭环反馈设计——世界模型在虚拟驾驶中的表现直接反馈到模型的进一步训练中。

## 关键特征
- **数据规模**: CARLA 12 个城仿+ nuScenes 真车数据、闭环反馈训练数。
- **数据模式**: RGB 图像、深度图、语义分割、驾驶动作、碰撞违规反馈信号
- **主要指标**: 驾驶分数、人类先验一致性、世界模型预测精度、策略迭代提。
- **领域**: 世界模型训练框架、人类驾驶先验、闭环自动驾驶学。

## 与世界模型的关系
DrivingDojo 为视觉世界模型在自动驾驶中的应用提供了一个训练道场"——系统地连接了世界模型的生成能力和自动驾驶的决策需求。该框架展示了世界模型如何通过与环境的持续交互（而非静态离线数据）来提升自身质量，这种"世界模型通过与世界互动来学习"的范式与人类认知过程非常类似。DrivingDojo 的人类先验注入"机制也为世界模型引入行为合理性约束提供了技术路线。
