...
该方法的另一个关键设计是显式方差正则化（Explicit Variance Regularization）——通过正则化项防止表示坍缩，无需像对比学习那样手动构造正负样本对。实验结果表明AD-L-JEPA在GPU小时上节省1.9x-2.7x，在GPU内存上节省2.8x-4x。

## 技术架构

**Vision Encoding（视觉编码）**：使用LiDAR点云的体素化编码器，将3D点云转化为BEV特征图。AD-L-JEPA采用上下文编码器和目标编码器的双编码器设计。
...
- 首个面向自动驾驶LiDAR数据的JEPA预训练框架
- BEV嵌入空间预测（而非点云生成），更高效
- 显式方差正则化替代对比学习的正负样本构造
- 在KITTI3D、Waymo、ONCE三大数据集上验证
- GPU资源效率：1.9x-2.7x GPU小时节省，2.8x-4x内存节省
- ONCE 100K帧预训练获得1.61 mAP增益，超越其他方法500K帧的效果

## 关键创新点

- 首个基于JEPA的自动驾驶LiDAR预训练方法
- BEV嵌入预测：非生成、非对比的第三条路径
- 显式方差正则化：替代对比对的手动构造
- 显著的计算资源和内存效率提升
- 在大规模数据集上验证负迁移问题的缓解
...
- KITTI3D、Waymo、ONCE一致改进
- ONCE 100K帧预训练（1.61 mAP增益）超越其他方法500K帧效果
- ONCE 500K帧（2.98 mAP增益）超越其他方法1M帧效果
- 1.9x-2.7x GPU小时节省 vs Occupancy-MAE
