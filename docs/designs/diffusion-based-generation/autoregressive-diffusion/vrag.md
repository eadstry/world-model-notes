...
VRAG（Video Retrieval Augmented Generation，Princeton）深入分析了视频世界模型的两个根本性挑战：(1) **复合误差**（compounding errors）在自回归视频生成中根本上是不可消除的——每一帧的微小误差在自回归循环中逐步放大；(2) **记忆机制不足**导致世界模型失去时空连贯性。

VRAG 的实验揭示了关键发现：朴素的自回归生成（即使扩展上下文窗口）和基于检索增强生成（RAG）对视频生成效果有限——主要原因是当前视频模型的**上下文学习能力有限**，无法有效利用额外提供的参考信息。
...
- VRAG 框架：视频检索增强生成 + 显式全局状态条件
- 复合误差分析：揭示自回归视频生成误差传播的本质
- 大规模评估基准：为视频世界模型提供全面对比
- 开源页面：sites.google.com/view/vrag

## 关键创新点

- 系统性揭示自回归视频生成中复合误差的不可消除性
- 发现当前视频模型的上下文学习能力有限（RAG 效果不佳）
- 显式全局状态条件替代隐式上下文学习
- 视频检索增强框架显著减少长时误差
- 建立全面的视频世界模型评估基准
