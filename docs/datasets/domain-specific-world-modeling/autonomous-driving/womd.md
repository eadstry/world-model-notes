...
## 简介
Waymo Open Motion Dataset (WoMD) 是 Waymo Open Dataset 的运动预测专项版本，专注于自动驾驶中多智能体运动预测和交互建模任务。WoMD 包含超过 100,000 个真实驾驶场景（每个 9 秒长，20Hz 采样），每个场景都包含 1 个"自车"目标代理和多达 8 个其他相邻交通参与者（车辆、行人、骑行者等）的完整轨迹数据。数据集覆盖美国多个城市的真实驾驶数据，附带高精度地图。
...
## 关键特征
- **数据规模**: 100K+ 交互场景、每场景 9 秒（20Hz）、50K+ 多小时驾驶数据
- **数据模态**: 所有交通参与者的 2D 轨迹、高精地图（车道、交通灯）、3D 物体信息
- **主要指标**: minADE、minFDE、Miss Rate（MR）、多智能体预测一致性
- **领域**: 多智能体运动预测、交互建模、自动驾驶行为预测

## 与世界模型的关系
WoMD 对世界模型研究的关键意义在于其"多智能体交互预测"的定义——这恰恰是世界模型在复杂场景中最核心的挑战。世界模型必须模拟的不仅是自车如何与环境互动，更是环境中各个智能体如何相互影响和共同演化。WoMD 的 9 秒长预测窗口也对世界模型的时间建模能力提出了严格要求。基于 WoMD 训练的多智能体预测模型已被广泛用作驾驶世界模型的"行为预测模块"。

## 代表性用途
- Waymo Sim Agents Challenge 的核心官方数据集
- MTR++ (2023) 在 WoMD 上实现多智能体运动预测 SOTA
- 被集成到端到端自动驾驶世界模型的预测模块中
- 驾驶场景交互式世界模型的基础训练和评测数据
